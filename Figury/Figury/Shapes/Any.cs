...
namespace Figury.Shapes
{
    public class Any : Shape
    {
        ...
        override public double Area()
        {
            double area = 0.5;
            double sum = 0;
            sum += pointList[0].X * (pointList[1].Y - pointList[pointList.Count - 1].Y);//skrarny
            sum += pointList[pointList.Count - 1].X * (pointList[0].Y - pointList[pointList.Count - 2].Y);//skrarny
